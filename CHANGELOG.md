...
- **Projections and sagas no longer skip events stamped slightly earlier by a peer node.** Offsets are event timestamps taken from the persisting node's clock, and a pull committed the timestamp of the newest event it saw, so an event stamped a few milliseconds earlier by a peer whose clock lags, or committed to the store just after the pull passed its timestamp, fell behind the committed offset and was never delivered. The runner now reads 100 ms behind the current time: an event is delivered only once it is at least that old, and a held-back event triggers another pull once it has aged, so events persisted on the local node keep their low latency. Skew beyond that window is not covered; keep node clocks synchronized. The README now also states the ordering guarantee: events of one entity are delivered in order, events of different entities are not.

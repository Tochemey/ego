...
- **A failed event write is handed to the entity's supervisor instead of silently stopping the entity.** After a store error the event-sourced actor shut itself down, a voluntary stop the supervisor never saw, so `SendCommand` failed with "actor not found" until the application called `Entity` again. The actor now replies with the error and fails the turn, and the directive configured with `WithSupervisorDirective` decides: the default `RestartDirective` replays the journal on the same PID, so the entity keeps serving commands from the state the store actually holds even after a partially committed write, while `StopDirective` stops it. The batched write path behaves the same once every stashed caller has received its error reply.

- **A missing extension no longer crashes the process.** Every eGo actor resolved its required extensions in `PreStart` with a bare type assertion. Go-Akt releases the extensions while the actor system shuts down and does not recover a panic in `PreStart`, so an entity, saga, projection, or writer that started during shutdown panicked with `interface conversion: extension.Extension is nil` and took the process down. The lookups now return an error wrapping `ErrMissingRequiredExtensions`, which Go-Akt reports as a start-up failure.
...
- **Projections and sagas no longer skip an event that reaches the store after a newer one.** An event was stamped when its command arrived and written afterwards, and a pull committed the timestamp of the newest event it saw, so when two entities' writes landed out of timestamp order the older event fell behind the committed offset and was never delivered. Events are now stamped by the writer just before the write, with the events of one write a nanosecond apart, which shrinks that window to the write itself. The same happened to an event stamped a few milliseconds earlier by a peer node whose clock lags. The runner now reads 100 ms behind the current time: an event is delivered only once it is at least that old, and a held-back event triggers another pull once it has aged, so events persisted on the local node keep their low latency. Write latency or clock skew beyond that window is not covered; keep node clocks synchronized. The README now also states the ordering guarantee: events of one entity are delivered in order, events of different entities are not.

- **Polling the journal no longer scales with the number of projections and sagas on a node.** Every projection and every saga runner asked the store for its shard offsets on each pull, so a node hosting many sagas issued that query once per saga per pull interval. The answer is now shared by every runner on the node for a quarter of a second, or until a local write, or a runner nudged by one, makes it stale, so a node issues at most a few of those queries per second however many runners it hosts. Events persisted on the node are still noticed immediately; events persisted by a peer can take up to that quarter of a second longer than before.

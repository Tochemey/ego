...
- **`Engine.Saga` requires an offset store.** A saga now reads the journal and records how far it has read, so a `Config` without `WithOffsetStore` makes `Engine.Saga` return `ErrOffsetStoreRequired`. Projections already need one.
- **Saga handlers must be idempotent.** Journal delivery is at-least-once: `SagaBehavior.HandleEvent` sees an event again when the saga restarts before its progress was recorded, and `SagaBehavior.Compensate` commands are resent when a saga restarts while compensating. Handlers that assumed exactly-once delivery need adjusting.
- **`offsetstore.OffsetStore` gains `DeleteOffset(ctx, projectionName)`.** It removes a projection's offsets across all shards; deleting offsets that do not exist is not an error. The SQL stores implement it as a delete by projection name.
- **`persistence.StateStore` gains `DeleteState(ctx, persistenceID, version)`.** With a version above zero the record is kept as a tombstone at that version with its state removed, so versions keep increasing across the deletion; with version zero the record is removed entirely. Deleting a state that does not exist is not an error.

### ✨ Features

- **Saga offsets can be removed once a saga settles.** `Engine.Saga` accepts `SagaOption` values. `WithOffsetRemoval()` deletes the rows a saga records under `ego.saga.<saga id>` when it reaches `SagaCompleted` or `SagaFailed`. Projection offsets are never touched, and a saga that is still running always resumes from its offsets.
- **Durable-state entities can be deleted.** A command handler deletes the entity by returning `egopb.DeletedState` as the new state with the next version. The store keeps a tombstone at that version, subscribers see the deletion under it, and the entity continues from its initial state, so the versions consumers observe keep increasing. `EraseEntity(full=true)` now removes the durable-state record as well.
- **Crypto-shredding through a key store.** `WithKeyStore` registers the `encryption.KeyStore` that `EraseEntity(full=false)` deletes an entity's key through. Without one, erasure returns `ErrKeyStoreRequired` instead of silently succeeding.
- **Bounded publishers.** Each delivery runs under a publish timeout, 30 seconds by default and configurable with `WithPublishTimeout`. Transient failures are retried with backoff, each publisher queue holds at most 10,000 payloads, and every drop is logged and counted on the new `ego.publisher.dropped.total` metric, labelled by publisher ID.

### 🐛 Bug Fixes

#### Sagas

- **Sagas see the events of every cluster node.** They consumed the in-process stream, which only carries events persisted on their own node, so a saga coordinating entities spread over a cluster silently missed steps. Sagas are now fed from the journal from the moment they first ran, with the local stream kept as a low-latency nudge.
- **Saga status survives restarts and is actually reported.** Recovery reset every saga to running, and `Engine.SagaStatus` never set `SagaInfo.Status`. Transitions are now journaled as `egopb.SagaStatusChanged`, replayed on recovery, and queried with `egopb.GetSagaStatus` and `egopb.SagaStatusReply`.
- **Compensation completes, and every step of it is attempted.** Compensation used to stop at the first unreachable participant, treat an error reply as compensated, and lose its pending count on restart. Every compensation command is now sent, each confirmed one is journaled as `egopb.SagaCompensationConfirmed`, a recovered saga resends only the unconfirmed ones, and the saga settles on `SagaFailed` if any participant was unreachable or rejected its compensation, otherwise on `SagaCompleted`.
- **Handler errors are retried and then surfaced.** A failing `HandleEvent`, `HandleResult`, or `HandleError` was logged and dropped, stalling the saga forever. Behavior calls and the saga's own journal writes are retried with backoff; exhausted retries move the saga to `SagaFailed`.
- **Sagas stay responsive while a participant is slow.** Participant calls run off the actor goroutine and their outcomes come back through the mailbox. A reply arriving after the saga has settled or started compensating is dropped.
- **Saga timeouts survive restarts.** The timeout was re-armed with its full duration on every start. The start time is now journaled with the first status transition, the remaining duration is armed instead, and a deadline that passed while the saga was down fires immediately.
- **Saga events are journaled on the saga's own shard.** They were written with shard zero, so every projection processed them and shard zero carried disproportionate load. The shard is now derived from the saga ID like an entity's, and projections skip eGo's framework events, recognised by their protobuf package.
- **A failed write leaves the saga as the journal has it.** Events were applied and the sequence counter bumped before the write, so a store error left the saga ahead of its journal. Events are now committed only after the write succeeds, and marshalling errors are no longer discarded.
- **A settled saga no longer polls the journal after a restart**, so it neither pulls events it would discard nor rewrites offsets it deleted with `WithOffsetRemoval`.

#### Entities

- **A failed event write is handed to the supervisor instead of silently stopping the entity.** The actor used to shut itself down, so `SendCommand` failed with "actor not found" until `Entity` was called again. The actor now replies with the error and fails the turn, and the `WithSupervisorDirective` setting decides: the default `RestartDirective` replays the journal on the same PID, `StopDirective` stops it. Batched writes behave the same once every stashed caller has its error reply.
- **Durable-state entities no longer advance their in-memory state on a failed write.** The write now happens first and the entity's state, version, and timestamp are updated only once it succeeds, matching event-sourced entities.
- **A missing extension no longer crashes the process.** Every eGo actor resolved its extensions with a bare type assertion, which panicked when an actor started during shutdown. The lookups now return an error wrapping `ErrMissingRequiredExtensions`, reported as a start-up failure.
- **`EraseEntity` erases.** With `full=false` it did nothing; with `full=true` it left the encryption key and the live actor in place. Both modes now stop the live entity first, then delete the key, and `full=true` also deletes events, snapshots, and durable state.

#### Projections and runners

- **Start and reset offsets are read in the right unit.** Offsets are nanosecond timestamps, but `projection.Options.StartOffset`, `ResetOffset`, and `Engine.RebuildProjection` converted with `UnixMilli`, so every reset replayed the whole journal. All three now use `UnixNano`. A start offset is also a floor a shard resumes from rather than being re-applied on every pull, which made a projection reprocess the same events forever.
- **An event that reaches the store after a newer one is no longer skipped.** Events are now stamped by the writer just before the write, and the runner reads 100 ms behind the clock, delivering an event only once it is at least that old while re-pulling as held-back events age. Clock skew beyond that window is not covered; keep node clocks synchronized. Ordering is documented: events of one entity arrive in order, events of different entities may not.
- **Journal polling no longer scales with the number of runners on a node.** Shard offsets were queried once per projection and per saga on every pull. The answer is now shared by every runner on the node for a quarter of a second, or until a local write makes it stale.

#### Testkit and documentation

- **The testkit stores behave like real ones.** `testkit.OffsetStore.ResetOffset` panicked as soon as the store held a row, and `testkit.EventStore.GetShardEvents` applied its limit before sorting, returning one event too many and not necessarily the oldest.
- **The README lists the ego-contrib stores that exist.** MongoDB stores were listed but never shipped; the Persistence section shows the actual coverage per backend.

### 📦 Examples

- **The cluster example runs a fund-transfer saga.** `POST /transfers/{id}` starts a `FundTransferSaga` that debits the source account, credits the destination, and refunds the source when a step is rejected; `GET /transfers/{id}` reports its status, outcome, and failure reason from any pod. `make test` now covers a successful transfer, a refunded one, and a rejected debit, and the README explains which ID belongs to the saga, the transfer entity, and the accounts.
- **The cluster example registers its behaviors with `WithEntityKinds`.** Spawns placed on a peer failed with "dependency type is not registered" and only succeeded on retry. Its PostgreSQL offset store implements `DeleteOffset`, and its dependencies are upgraded, including goakt v4.5.5, pgx v5.11.0, and otelhttp v0.71.0.
- **`example/examplepb` gains `StartTransfer`, `TransferFailed`, and `TransferState.failure_reason`** for the transfer saga.

### 📖 Migration

1. Add `DeleteOffset` to every `offsetstore.OffsetStore` implementation and `DeleteState` to every `persistence.StateStore` implementation.
2. Configure `ego.WithOffsetStore` on any `Config` used to start sagas.
3. Make `SagaBehavior.HandleEvent` and every compensation command idempotent, for example by checking the saga state before acting and keying participant commands by saga ID.
4. If you relied on `EraseEntity(full=false)` returning nil without a key store, configure `WithKeyStore` or expect `ErrKeyStoreRequired`.
